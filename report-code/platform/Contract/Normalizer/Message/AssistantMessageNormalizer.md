# AssistantMessageNormalizer 分析报告
...
## normalize() 方法输出格式
- 输出: `{role: "assistant", content: string|null, tool_calls?: array}`
- 若消息包含工具调用，序列化 `tool_calls` 数组（需要 `NormalizerAwareTrait` 委托给 `ToolCallNormalizer`）
