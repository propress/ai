...
| 能力 | 说明 |
|------|------|
| **统一的 AI 平台接口** | 通过 Platform 组件，使用相同的 API 调用 33 个 AI 平台（OpenAI、Anthropic、Gemini 等） |
| **智能代理框架** | 通过 Agent 组件构建能使用工具、协作完成任务的 AI 代理 |
| **向量存储与 RAG** | 通过 Store 组件，支持 24 个向量数据库后端，轻松构建检索增强生成应用 |
| **多轮对话管理** | 通过 Chat 组件，管理对话历史，支持 10 个存储后端 |
...
| **第 9-14 章：入门实战** | 智能客服、多轮对话、结构化提取、多模态、翻译、流式对话 |
| **第 15-19 章：进阶实战** | 工具增强、RAG 知识库、多代理协作、联网搜索、记忆系统 |
| **第 20-24 章：高级实战** | 高可用架构、私有化部署、内容审核、多智能体团队、企业知识库 |
| **第 25 章：架构与最佳实践** | 设计模式、错误处理策略、安全防护、可观测性 |
| **第 26 章：附录** | API 速查表、版本升级指南、常见问题解答 |
...
OpenAI、Anthropic 等 33 个 AI 平台的能力。
```
