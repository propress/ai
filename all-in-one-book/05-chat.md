...
### Chat 组件 vs 手动管理 MessageBag

| 维度 | Chat 组件 | 手动管理 MessageBag |
|------|----------|-------------------|
| **持久化** | 自动保存/加载对话历史 | 需手动实现 |
| **会话隔离** | 内置多会话管理 | 需自行实现 |
| **代码量** | 少（2-3 行） | 多（需要存储逻辑） |
| **灵活性** | 受限于 `submit()` 接口 | 完全控制 |
| **流式支持** | 不直接支持 | 可通过 Platform 实现 |
| **适用场景** | 标准多轮对话 | 需要自定义对话逻辑 |

### submit() vs 流式响应 —— 何时选择？

| 维度 | `Chat::submit()` | Platform 流式 + 手动存储 |
|------|------------------|-------------------------|
| **实现复杂度** | ⭐ 简单 | ⭐⭐⭐ 较复杂 |
| **用户体验** | 等待完整回复 | 逐字显示，感知更快 |
| **适用场景** | API 后端、批处理 | Web 前端、实时聊天 |
| **对话持久化** | 自动 | 需手动实现 |
...
> ⚠️ **常见错误：传入错误的消息类型**
>
> `Chat::submit()` 的参数类型是 `UserMessage`，不是通用的 `Message`：
> ```php
> // ❌ 错误
> $chat->submit(new Message('user', '你好'));
>
> // ✅ 正确
> use Symfony\AI\Platform\Message\UserMessage;
> $chat->submit(new UserMessage(new Text('你好')));
> ```
