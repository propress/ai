...
$response = $platform->invoke($messages, 'llama3.1');
```
...
// 4. 第一轮对话
$response = $chat->submit($conversationId, '我想退货');
echo $response->asText();
// "好的，请提供您的订单号，我来帮您查询退货资格。"

// 5. 第二轮对话——AI 自动获得之前的上下文
$response = $chat->submit($conversationId, '订单号是 ORD-20240115');
echo $response->asText();
// "已查到订单 ORD-20240115，购买于 2024-01-15，在 30 天退货期内..."

// 6. 第三轮——继续追问
$response = $chat->submit($conversationId, '运费谁承担？');
echo $response->asText();
...
$options = [                          1. 检测到 'output' => ProductInfo::class
    'output' => ProductInfo::class    2. 使用 JsonSchemaFactory 生成 JSON Schema
...
$response = $platform->invoke($messages, $model, [
    'output' => ProductImageAnalysis::class,
]);
