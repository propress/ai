...
Symfony AI 的事件系统分布在多个组件中。以下是所有可监听事件的完整清单：

#### Platform 事件

| 事件类 | 触发时机 | 用途 |
|--------|---------|------|
| `InvocationEvent` | 调用 AI 模型前 | 日志记录、请求修改、计量 |
| `ResultEvent` | 结果返回后 | 结果处理、Token 统计、告警 |

#### Agent 事件

| 事件类 | 触发时机 | 用途 |
|--------|---------|------|
| `ToolCallRequested` | 工具执行前 | 安全审查、结果缓存、权限检查 |
| `ToolCallingEvent` | 工具执行后 | 执行日志、结果修改 |

#### Store 事件

| 事件类 | 触发时机 | 用途 |
|--------|---------|------|
| `PreQueryEvent` | 向量查询前 | 查询改写、过滤条件注入 |
| `PostQueryEvent` | 查询结果返回后 | 结果重排序、过滤、缓存 |
...
$dispatcher->addListener(InvocationEvent::class, function (InvocationEvent $event) {
    $logger->info('Invoking model', ['model' => $event->getModel()]);
});
...
> 💡 **提示**：在 AI Bundle 中，所有事件监听器通过 Symfony 的标准 `#[AsEventListener]` 属性自动注册，无需手动配置。
...
### 6.1A 使用 InMemoryPlatform 测试

`InMemoryPlatform` 是 Symfony AI 提供的测试专用 Platform 实现，无需真实 API 调用即可验证业务逻辑。
...
$platform = new InMemoryPlatform('这是一个固定的 AI 回复');

$result = $platform->invoke('gpt-4o', '你好');
echo $result->asText(); // "这是一个固定的 AI 回复"
```

#### 动态响应 —— Closure 回调

```php
$platform = new InMemoryPlatform(function (string $model, mixed $input, array $options): string {
    // 根据输入动态返回
    if (str_contains((string) $input, '天气')) {
...
| 工具 | 测试层级 | 适用场景 |
|------|---------|---------|
| `InMemoryPlatform` | Platform 层 | 测试使用 Platform 的代码，需要验证调用参数 |
| `MockAgent` | Agent 层 | 测试依赖 Agent 的上层代码，无需关心内部细节 |

> 💡 **建议**：当测试 Agent 行为时使用 `MockAgent`；当测试直接使用 `Platform::invoke()` 的代码时使用 `InMemoryPlatform`。
